...
const OfferFormModal = ({ open, initialData, onClose, onSave }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-end bg-slate-950/50 p-3 backdrop-blur-sm sm:items-center sm:justify-center">
      <form onSubmit={handleSubmit} className="w-full max-w-2xl overflow-hidden rounded-[32px] bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-4 py-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">Offer</p>
            <h2 className="mt-1 text-xl font-black text-slate-950">{initialData ? "Edit Offer" : "Add Offer"}</h2>
          </div>
        ...
        <div className="max-h-[78vh] overflow-y-auto px-4 py-4">
          ...
          <label className="mt-3 grid gap-2">
            <span className="text-xs font-black uppercase tracking-wide text-slate-400">Banner text</span>
            <textarea
              rows="3"
