...
export const ORDER_STATUSES = ["placed", "whatsapp_pending", "confirmed", "preparing", "assigned", "picked_up", "on_the_way", "out_for_delivery", "delivered", "failed_delivery", "cancelled"];
...
export const ORDER_STATUS_META = {
  ...
  whatsapp_pending: { label: "WhatsApp Pending", note: "Waiting for WhatsApp confirmation", tone: "orange" },
...
export const normalizeOrder = (orderPayload = {}) => {
  ...
  const riderObject =
    (orderPayload.riderId && typeof orderPayload.riderId === "object" ? orderPayload.riderId : null) ||
    (orderPayload.deliveryPartnerId && typeof orderPayload.deliveryPartnerId === "object" ? orderPayload.deliveryPartnerId : null);
  const riderId = orderPayload.riderId && typeof orderPayload.riderId !== "object"
    ? orderPayload.riderId
    : orderPayload.deliveryPartnerId && typeof orderPayload.deliveryPartnerId !== "object"
      ? orderPayload.deliveryPartnerId
      : riderObject?._id || riderObject?.id || null;
  ...
  return {
    ...
    status,
    riderId,
    riderName: orderPayload.riderName || riderObject?.name || "",
    riderPhone: orderPayload.riderPhone || riderObject?.phone || "",
    riderVehicleType: orderPayload.riderVehicleType || riderObject?.vehicleType || riderObject?.vehicle || "",
    riderStatus: orderPayload.riderStatus || riderObject?.currentStatus || riderObject?.status || "",
    paymentStatus,
    trackingSteps,
  };
};
