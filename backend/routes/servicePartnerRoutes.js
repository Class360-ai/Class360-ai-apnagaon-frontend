...
router.get("/admin/service-partners", requireAuth, roleCheck("admin"), async (req, res) => {
  const partners = await ServicePartner.find({}).sort({ createdAt: -1 }).limit(200);
  return res.json(partners);
});

router.put("/admin/service-partners/:id/status", requireAuth, roleCheck("admin"), async (req, res) => {
